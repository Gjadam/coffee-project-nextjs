    ...
    return (
        <div data-aos='fade-right' className=' w-full'>
            ...
            <div className=" flex flex-col gap-5 my-5">
                <FormInput placeholder={'نام کاربری *'} type={'text'} onChange={(e) => setName(e.target.value)} value={name} />
                <FormInput placeholder={'شماره موبایل *'} type={'number'} onChange={(e) => setPhone(e.target.value)} value={phone} />
                <FormInput placeholder={'ایمیل *'} type={'email'} onChange={(e) => setEmail(e.target.value)} value={email} />
                <div className={` ${isRegisterWithPass ? 'opacity-100' : 'opacity-45'} transition-all duration-300`}>
                    <FormInput placeholder={'رمز عبور *'} type={'password'} onChange={(e) => setPassword(e.target.value)} isDisabled={isRegisterWithPass ? false : true} value={password} />
                </div>
            </div>
